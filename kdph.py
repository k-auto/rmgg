...
def _github_upload(token, repo_name, target_file, commit_message="Uploaded file.", topics=None, desc=None):
    ...
    try:
        ...
        repo.create_file(file_name, commit_message, content_str)

def _github_download(author, repo, branch, target_file, binary=False):
...
def mkpkg(folder, key=None, token=None):
    ...
    try:
        ...
        _github_upload(token, folder, package_enc, "Knexyce Package manifested.")
...
def getpkg(author, package, key=None, location=None):
    ...
    try:
        _github_download(author, package, "main", package_enc)
    except:
        pass
    decrypt_file(package_enc, package_archive, key)
    _extract_archive(package_archive, location)
    os.remove(package_enc)
    os.remove(package_archive)
