...
def github_download(author, repo_name, branch, target_path, folder_path=False, binary=False):
    if folder_path:
        ...
        for item in items:
            if item["type"] == "dir":
                subfolder = os.path.join(target_path, os.path.basename(item["path"]))
                os.makedirs(subfolder, exist_ok=True)
                github_download(author, repo_name, branch, item["path"], folder_path=True, binary=binary)
            else:
