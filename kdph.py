...
def createkp(folder, key=None, location=None):
    ...
    if location is None:
        location = Path.cwd()
    kbuild_path = folder_path / "kbuild.py"
...
""")
    ...
    archive_folder(str(folder_path), archive_file)
...
def openkp(package, key=None, location=None):
    ...
    if location:
        ...
        location = Path(package_name).resolve()
    ...
    kbuild_path = location / "kbuild.py"
    subprocess.run([os.sys.executable, str(kbuild_path)], cwd=str(location), check=True)
